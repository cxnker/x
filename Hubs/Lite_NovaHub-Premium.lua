...
local Section = Tab1:AddSection({"V1.12"})
...
function VehicleTeleport:MostrarNotificacao(mensagem)
    pcall(function()
        game:GetService("StarterGui"):SetCore("SendNotification", {
            Title = "Aviso",
            Text = mensagem,
...
function VehicleTeleport:UpdateListVehicles()
    local VehicleList = self.Workspace:FindFirstChild("Vehicles")
    local VehiclesList = {}
    
    if VehicleList then
        for _, carro in ipairs(VehicleList:GetChildren()) do
            if carro.Name:match("Car$") then
                table.insert(VehiclesList, carro.Name)
            end
        end
    end
    return VehiclesList
    ...
    Callback = function(state)
        ...
        if state then
            if self.LocalPlayer.Character and self.LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
                ...
                VehicleTeleport:MostrarNotificacao("Personaje no encontrado!")
                return
            end
            ...
            spawn(function()
                ...
                if not vehiclesFolder then
                    VehicleTeleport:MostrarNotificacao("Vehiculo no encontrado!")
                    return
                end
...
local Dropdown = Tab5:AddDropdown({
    ...
    Default = nil,
    Options = VehicleTeleport:UpdateListVehicles(),
    ...
    Callback = function(state)
        if state then
            if not _G.SelectedVehicle or _G.SelectedVehicle == "" then
                VehicleTeleport:MostrarNotificacao("Ningun vehiculo seleccionado!")
                return
            end

            local vehiclesFolder = VehicleTeleport.Workspace:FindFirstChild("Vehicles")
            if not vehiclesFolder then
                VehicleTeleport:MostrarNotificacao("Vehiculo no encontrado en la carpeta!")
                return
            end
            ...
            if not vehicleSeat then
                VehicleTeleport:MostrarNotificacao("Asiento del vehiculo no encontrado!")
                return
            end
            ...
            VehicleTeleport:MostrarNotificacao("Camara ajustada para el vehiculo " .. _G.SelectedVehicle .. "!")
        else
            if VehicleTeleport.OriginalCameraSubject then
                ...
                VehicleTeleport:MostrarNotificacao("Camara restaurada!")
...
VehicleTeleport.Workspace:WaitForChild("Vehicles").ChildAdded:Connect(function()
    Dropdown:Set(VehicleTeleport:UpdateListVehicles())
end)
...
Tab5:AddButton({
    ...
    Callback = function()
        ...
        if vehicleSeat.Occupant then
            VehicleTeleport:MostrarNotificacao("Vehiculo en uso, no fue posible eliminarlo!")
            return
        end
        ...
        if VehicleTeleport.LocalPlayer.Character and VehicleTeleport.LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
            ...
            VehicleTeleport:MostrarNotificacao("Personaje del jugador no encontrado!")
            return
        end
        ...
        if success then
            ...
            VehicleTeleport:MostrarNotificacao("Vehiculo " .. _G.SelectedVehicle .. " Fue teletransportado al vacio!")
            VehicleTeleport:ExitCarAndReturn(originalPos)
        else
            VehicleTeleport:MostrarNotificacao("Fallo al sentarte en el vehiculo!")
        end
        VehicleTeleport:ToggleFallDamage(false)
    end
})
...
Tab5:AddButton({
    ...
    Callback = function()
        ...
        local vehiclesFolder = VehicleTeleport.Workspace:FindFirstChild("Vehicles")
        if not vehiclesFolder then
            VehicleTeleport:MostrarNotificacao("Vehiculo no encontrado en carpeta!")
            return
        end
        ...
        if vehicleSeat.Occupant then
            VehicleTeleport:MostrarNotificacao("El vehiculo esta en uso, teletransporte fallido!")
            return
        end
        ...
        if success then
            ...
            VehicleTeleport:MostrarNotificacao("Vehiculo " .. _G.SelectedVehicle .. " fue teletransportando a ti!")
            VehicleTeleport:ExitCarAndReturn(originalPos)
        else
            VehicleTeleport:MostrarNotificacao("Fallo al sentarte en el vehiculo!")
        end
...
Tab5:AddButton({
    ...
    Callback = function()
        ...
        for _, car in ipairs(cars) do
            ...
            if vehicleSeat and vehicleSeat.Occupant == nil then
                ...
                if success then
                    ...
                    VehicleTeleport:ExitCarAndReturn(originalPos)
                    VehicleTeleport:MostrarNotificacao("Vehiculo " .. car.Name .. " fue teletransportado a ti!")
                    task.wait(1)
                else
                    VehicleTeleport:MostrarNotificacao("No puedes sentarte en el vehiculo " .. car.Name .. "!")
                end
            else
                if vehicleSeat then
                    VehicleTeleport:MostrarNotificacao("Vehiculo " .. car.Name .. " Vehiculo en uso!")
                else
                    VehicleTeleport:MostrarNotificacao("Vehiculo " .. car.Name .. " Asiento no encontrado!")
                end
            end
        end
        ...
        VehicleTeleport:ToggleFallDamage(false)
        if #cars == 0 then
            VehicleTeleport:MostrarNotificacao("Ningun vehiculo disponible para teletransportar!")
        end
    end
})
...
createSoundDropdown("Selecciona un sonido", {
    ["Memes"] = {
...
local DropdownPlayerTab2 = Tab9:AddDropdown({
    ...
    Callback = function(selectedPlayerName)
        trollplayervalue = selectedPlayerName
...
Tab9:AddButton({
    ...
    Callback = function()
        local selectedPlayerName = trollplayervalue
...
----------------------------------------------------------------------------------------------------
local Section = Tab10:AddSection({"Paneles"})
...
local Section = Tab12:AddSection({"Mejoras graficas"})
Tab12:AddButton({"Shaders", function()
