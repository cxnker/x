...
local ScreenGui = Create("ScreenGui", "NOVA HUB", CoreGui)
...
local Notification = SetConfigs(Create("Frame", "Notification", ScreenGui), {
...
local Padding = SetConfigs(Create("UIPadding", "Padding", Notification), {
  PaddingLeft = UDim.new(0, 25),
  PaddingTop = UDim.new(0, 25),
  PaddingBottom = UDim.new(0, 50)
})

local ListLayout = SetConfigs(Create("UIListLayout", "ListLayout", Notification), {
  Padding = UDim.new(0, 15),
  VerticalAlignment = "Bottom"
})

function AddNotify(Configs)
  local Title = Configs.Title or "NOVA HUB"
  local text = Configs.Text or "Notify"
  local time = Configs.Time or 5
  
  local Frame1 = SetConfigs(Create("Frame", "Frame", Notification), {
    Size = UDim2.new(2, 0, 0, 75),
    BackgroundTransparency = 1
  })
  
  local Frame2 = SetConfigs(Create("Frame", "Notify", Frame1), {
    Size = UDim2.new(0, Notification.Size.X.Offset - 50, 0, 75),
    BackgroundColor3 = Configs_HUB.Hub,
    Position = UDim2.new(0, Notification.Size.X.Offset, 0, 0),
    AutomaticSize = "Y"
  })Corner(Frame2)
  ...
  TextButton.MouseButton1Click:Connect(function()
    ...
    {Position = UDim2.new(0, Notification.Size.X.Offset, 0, 0)})tween:Play()tween.Completed:Wait()
    Frame1:Destroy()
  end)
...
local Menu = SetConfigs(Create("Frame", "Start Menu", ScreenGui), {
...
local function SizeAdd(size, position, anchor)
  local Line = SetConfigs(Create("Frame", "Line", Menu), {
    Size = size,
    Position = position,
  ...
  if Configs_HUB.Stroke == "Rainbow" then
    ...
    Line.BackgroundColor3 = Configs_HUB.Stroke
  end
  return Line
end
...
function AddInfo(Configs)
  SetConfigs(Title, {
    Text = Configs.Title or "NOVA HUB",
...
function MainMobile(Configs)
  local name = Configs.Name or "ToggleM"
  local Callback = Configs.Callback or function() end
  local visible = Configs.Visible or false
  
  local ToggleMain = SetConfigs(Create("Frame", "ToggleM", ScreenGui), {
  ...
  local TextLabel = SetConfigs(Create("TextLabel", "TextLabel", ToggleMain), {
    Size = UDim2.new(1, 0, 0, 20),
    TextSize = 20,
    Font = Enum.Font.FredokaOne,
    TextColor3 = Configs_HUB.TextColor,
  ...
  local Button = SetConfigs(Create("TextButton", "Button", ToggleMain), {
